...
export class NavigationController {
  constructor() {
    ...
    // Navigation config
    this.config = (typeof AppConfig !== 'undefined' && AppConfig.NAVIGATION) ? AppConfig.NAVIGATION : {
  ...
  getScrollableParent(el) {
    ...
    while (cur && cur !== document.body) {
      ...
      const canScroll = (overflowY === 'auto' || overflowY === 'scroll') && cur.scrollHeight > cur.clientHeight + 1;
  ...
  setupWheelNavigation() {
    const handleWheel = (e) => {
      ...
      const canScroll = scrollContainer.scrollHeight > scrollContainer.clientHeight + this.config.SECTION_SCROLL_EDGE_PX;
      
      if (canScroll) {
        const maxScrollTop = scrollContainer.scrollHeight - scrollContainer.clientHeight;
        const atTop = scrollContainer.scrollTop <= this.config.SECTION_SCROLL_EDGE_PX;
        const atBottom = scrollContainer.scrollTop >= (maxScrollTop - this.config.SECTION_SCROLL_EDGE_PX);
      ...
      const now = Date.now();
      if (now - this.lastSectionScrollTs < this.config.SECTION_SCROLL_COOLDOWN_MS) return;
